...
static int circuit_deliver_create_cell(circuit_t *circ,
                                       ...
                                       int relayed);
static int onion_pick_cpath_exit_Mohsen(origin_circuit_t *circ, extend_info_t *exit, float lat, float lon,float Mylat, float Mylon);//Mohsen
static int onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit);
static crypt_path_t *onion_next_hop_in_cpath(crypt_path_t *cpath);
static int onion_extend_cpath(origin_circuit_t *circ);
static int onion_extend_cpath_Mohsen(origin_circuit_t *circ, float lat, float lon);//Mohsen
...
//Mohsen 25 Feb 2 PM--------------------------------------------
static int
onion_populate_cpath_Mohsen(origin_circuit_t *circ, float lat, float lon)
{
  int r;
 again:
  r = onion_extend_cpath_Mohsen(circ, lat,lon);
  if (r < 0) {
    log_info(LD_CIRC,"Generating cpath hop failed.");
    return -1;
  }
...
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit, int flags)
{
  ...
  circ->base_.sendme_flag = 1; //Mohsen
  circ->base_.rtt = 0; //Mohsen
  circ->base_.no_sendme = 0;//Mohsen
  circ->base_.number_of_sent_DATA = 0;//Mohsen
...
//Mohsen 25 Feb 2PM----------------------------------------------
origin_circuit_t *
circuit_establish_circuit_Mohsen(uint8_t purpose, extend_info_t *exit, int flags,float lat, float lon, float Mylat, float Mylon)
{
  origin_circuit_t *circ;
  int err_reason = 0;
  ...
  circ->base_.number_of_sent_DATA = 0;//Mohsen
// Mohsen 8 Feb-------------------------------------
// Now we find around me circuits change "onion_pick_cpath_exit" lat and lon for the others
  if (onion_pick_cpath_exit_Mohsen(circ, exit, lat,lon,Mylat,Mylon) < 0 ||//Mohsen
      onion_populate_cpath_Mohsen(circ,Mylat,Mylon) < 0) {
    circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_NOPATH);
    return NULL;
  }
                            ...
                            int relayed)
{
  ...
  if (CIRCUIT_IS_ORIGIN(circ)) {
    ...
    if (TO_ORIGIN_CIRCUIT(circ)->cpath->state == CPATH_STATE_CLOSED) {
      ...
      // Mohsen----------------------------------------
     //-log_debug(LD_CIRC,"Hey Mohsen: extended flag set to zero");
      circ->no_extended = 0;
    }
...
//Mohsen ---------------------------------------------------------
        tor_gettimeofday(&circ->base_.end);
        //tor_gettimeofday(&circ->base_.sendme1);
        circ->base_.sendme_flag = 1;
        circ->base_.counter = 0;
        circ->base_.rtt = tv_mdiff(&circ->base_.start, &circ->base_.end); 
        if (circ->base_.rtt > 0){
        //-log_info(LD_CIRC,"Hey Mohsen: end time set and RTT = %ldmsec.", circ->base_.rtt);       
        } else {
        log_info(LD_CIRC,"Hey Mohsen: ERROR RTT less than zero");
        }            
//------------------------------------------------------------------
        /*
...
//Mohsen-------------------------------------------------------
        if (circ->base_.no_extended == 1) {
        tor_gettimeofday(&circ->base_.start);
        //-log_info(LD_CIRC,"Hey Mohsen: start time set");
        }
//-------------------------------------------------------------
...
//Mohsen 25 Feb 2 Pm------------------------------------
static const node_t *
choose_good_exit_server_general_Mohsen(int need_uptime, int need_capacity, float lat, float lon, float Mylat, float Mylon)
{
  int *n_supported;
  int n_pending_connections = 0;
  smartlist_t *connections;
...
// Mohsen: 4 Feb --------------------------------------------------------------
    node = node_sl_choose_by_bandwidth_Mohsen(supporting, WEIGHT_FOR_EXIT,lat,lon,0,0);
//-------------------------------------------------------------------------------
    smartlist_free(supporting);
  } else {
    /* Either there are no pending connections, or no routers even seem to
    ...
    if (best_support == -1) {
      if (need_uptime || need_capacity) {
        log_info(LD_CIRC,
                 ...
                 need_uptime?", stable":"");
        tor_free(n_supported);
        return choose_good_exit_server_general_Mohsen(0, 0, lat, lon,Mylat,Mylon);// Mohsen: 4 Feb:  You should change it 
      }
      log_notice(LD_CIRC, "All routers are down or won't exit%s -- "
                 "choosing a doomed exit at random.",
                 options->ExcludeExitNodesUnion_ ? " or are Excluded" : "");
...
//Mohsen 25 Feb 2 PM--------------------------
static const node_t *
choose_good_exit_server_Mohsen(uint8_t purpose,
                        int need_uptime, int need_capacity, int is_internal, float lat, float lon, float Mylat, float Mylon)
{
  const or_options_t *options = get_options();
  router_crn_flags_t flags = CRN_NEED_DESC;
  if (need_uptime)
  ...
  switch (purpose) {
    case CIRCUIT_PURPOSE_C_GENERAL:
      ...
      if (is_internal) /* pick it like a middle hop */
        ...
        return choose_good_exit_server_general_Mohsen(need_uptime,need_capacity, lat,lon,Mylat,Mylon);
    case CIRCUIT_PURPOSE_C_ESTABLISH_REND:
      if (options->AllowInvalid_ & ALLOW_INVALID_RENDEZVOUS)
        flags |= CRN_ALLOW_INVALID;
      return router_choose_random_node(NULL, options->ExcludeNodes, flags);
...
//Mohsen 25 Feb 2PM----------------------------------------------------------
static int
onion_pick_cpath_exit_Mohsen(origin_circuit_t *circ, extend_info_t *exit, float lat, float lon, float Mylat, float Mylon)
{
  cpath_build_state_t *state = circ->build_state;
  circ->lat=lat;
  circ->lon=lon;
...
//Mohsen 8 Feb-------------------------------------------------
    const node_t *node =
      choose_good_exit_server_Mohsen(circ->base_.purpose, state->need_uptime,
                              state->need_capacity, state->is_internal, lat, lon,Mylat,Mylon);

    if (!node) {
      log_warn(LD_CIRC,"failed to choose an exit server");
      return -1;
    }
    //log_info(LD_DIR,"\nHey Mohsen: Exit node->lat = %f node->lon %f\n",node->lat,node->lon);
    ...
    //-log_info(LD_DIR,"Hey Mohsen: Exit %s IP = %s\n",nickname,circ->base_.exit_ip);
//--------------------------------------------------------------------------------
    exit = extend_info_from_node(node, 0);
    tor_assert(exit);
  }
...
//Mohsen 25 Feb 2PM---------------------------------------------
static const node_t *
choose_good_middle_server_Mohsen(uint8_t purpose,
                          cpath_build_state_t *state,
                          crypt_path_t *head,
                          int cur_len)
{
...
//      nodelist_add_node_and_family(excluded, r);
  ...
  if (options->AllowInvalid_ & ALLOW_INVALID_MIDDLE)
    flags |= CRN_ALLOW_INVALID;
  //-log_info(LD_CIRC,"Hey Mohsen:entry_lat = %f entry_lon = %f exit_lat = %f exit_lon = %f", entry_lat, entry_lon, exit_lat,exit_lon);
  choice = router_choose_random_node_Mohsen(excluded, options->ExcludeNodes, flags,entry_lat,entry_lon,exit_lat,exit_lon);
  smartlist_free(excluded);
  return choice;
}
...
//Mohsen 25 Feb 2PM--------------------------------------------------
static int
onion_extend_cpath_Mohsen(origin_circuit_t *circ, float lat, float lon)
{
  uint8_t purpose = circ->base_.purpose;
  cpath_build_state_t *state = circ->build_state;
  int cur_len = circuit_get_cpath_len(circ);
...
//Mohsen 25 Feb 2PM --------------------------------------------------------------------
    const node_t *r = choose_good_entry_server_Mohsen(purpose, state, lat,lon);
...
//Mohsen 25 Feb 2PM --------------------------------------------------------------------
    ...
    const char *nickname = node_get_nickname(r);
    //-log_info(LD_DIR,"\nHey Mohsen: Entry %s IP = %s\n",nickname,circ->base_.entry_ip);
//-------------------------------------------------------------------------
      info = extend_info_from_node(r, server_mode(get_options()) == 0);
      tor_assert(info);
    }
    ...
    const node_t *r =
      choose_good_middle_server_Mohsen(purpose, state, circ->cpath, cur_len);//Mohsen
...
//Mohsen 25 Feb 2PM --------------------------------------------------------------------
    ...
    const char *nickname = node_get_nickname(r);
    //-log_info(LD_DIR,"\nHey Mohsen: Middle %s IP = %s \n",circ->base_.middle_ip);
//-------------------------------------------------------------------------
      info = extend_info_from_node(r, 0);
      tor_assert(info);
    }
...
const node_t *
choose_good_entry_server_Mohsen(uint8_t purpose, cpath_build_state_t *state, float lat, float lon)
{
  const node_t *choice;
  smartlist_t *excluded;
  const or_options_t *options = get_options();
...
*/
  ...
  choice = router_choose_random_node_Mohsen(excluded, options->ExcludeNodes, flags,lat,lon,exit_lat,exit_lon);
