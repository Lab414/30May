...
circuit_get_best(const entry_connection_t *conn,
                 ...
                 int need_uptime, int need_internal)
{
    //-log_info(LD_CIRC,"Hey Mohsen: soket addr %s state %u",conn->socks_request->address,conn->edge_.base_.state); 
  circuit_t *circ;
  ...
  int counter = 0;
  long rtt = 100000000;
  origin_circuit_t *origin_circ_best = NULL;
  int i = 0;
  int h = 0; 
  ...
  log_info(LD_CIRC," Hey Mohsen: we have %d circuits",ii); 
...
//Mohsen------------------------------------------
  ...
  counter = 0;
  rtt = 100000000;
  origin_circ_best = NULL;
  //log_info(LD_CIRC,"\n\n Hey Mohsen: we have %d circuits",ii); 
  tor_addr_t addr_moh;

  if (tor_addr_parse(&addr_moh, conn->socks_request->address)>0){

  GeoIP  *gi;
  GeoIPRecord *gir;
  gi = GeoIP_open("/home/mm/GeoIP-1.4.6/data/GeoLiteCity.dat", GEOIP_INDEX_CACHE);

  gir = GeoIP_record_by_name(gi, conn->socks_request->address);
  if (!gir){
  log_info(LD_CIRC," Hey Mohsen:gir is NULL"); 
  } // if
//*************************GeoIP***********************************
  c_i = tor_malloc_zero(sizeof(circuit_idx)*ii);
  if (c_i == NULL) {
  log_info(LD_CIRC," Hey Mohsen:c_i is NULL"); 
  }//if

//*************************fill the array************************** 
  for (circ_m=global_circuitlist;circ_m;circ_m = circ_m->next) {
     c_i[h].ID = circ_m-> n_circ_id;
     c_i[h].distance = compute_distance(circ_m->lat, circ_m->lon,((gir)? gir->latitude: MYLAT), ((gir)? gir->longitude: MYLON));
     //c_i[h].rrt = compute_distance(circ_m->lat, circ_m->lon,gir->latitude, gir->longitude);
      h++;
     }//for
//*************************sort***********************************
  qsort (c_i, ii, sizeof(circuit_idx), compare_d);
//*************************sort***********************************



  for (i=0; i<ii; i++){
    circ = circuit_get_by_id(c_i[i].ID);
   ...
   counter++;
   //log_info(LD_REND|LD_CIRC, "counter %d",counter);
   if (rtt > circ->rtt && circ->rtt != 0) {
      rtt = circ->rtt;
      log_info(LD_REND|LD_CIRC, "RTT %ld counter %d",rtt, counter);
      origin_circ_best  = origin_circ;
   } //if

   if ( counter == MIN_NUMBER_OF_CIRC_FOR_EACH_DEST || ii < MIN_NUMBER_OF_CIRC_FOR_EACH_DEST || (counter < MIN_NUMBER_OF_CIRC_FOR_EACH_DEST && i ==(ii - 1) )){
   log_info(LD_REND|LD_CIRC, "circuit lat %f lon %f stream lat %f lon %f, RTT %ld", circ->lat, circ->lon, ((gir)? gir->latitude: MYLAT), ((gir)? gir->longitude: MYLON),rtt);

   GeoIPRecord_delete(gir);
   GeoIP_delete(gi);
   free (c_i);

   return origin_circ_best;
   } //if
  }//for
log_info(LD_REND|LD_CIRC, "There is no circuit");
...
//*************************fill the array************************** 
  ...
  for (circ_m=global_circuitlist;circ_m;circ_m = circ_m->next) {
     c_i[h].ID = circ_m-> n_circ_id;
     c_i[h].distance = compute_distance(circ_m->lat, circ_m->lon, MYLAT, MYLON);
     //c_i[h].rrt = compute_distance(circ_m->lat, circ_m->lon,gir->latitude, gir->longitude);
      h++;
     }//for
//*************************sort***********************************
...
//*************************sort***********************************
  ...
  for (i=0; i<ii; i++){
   ...
   if ( counter == MIN_NUMBER_OF_CIRC_FOR_EACH_DEST || ii < MIN_NUMBER_OF_CIRC_FOR_EACH_DEST || (counter < MIN_NUMBER_OF_CIRC_FOR_EACH_DEST && i ==(ii - 1) )){
   log_info(LD_REND|LD_CIRC, "circuit lat %f lon %f  RTT %ld", circ->lat, circ->lon,rtt);
   free (c_i);
   return origin_circ_best;
   } //if
  }//for
...
static void
circuit_predict_and_launch_new_Mohsen(void)
{
  circuit_t *circ;
  int num=0, num_internal=0, num_uptime_internal=0, i = 0, j = 0;
  int hidserv_needs_uptime=0, hidserv_needs_capacity=1;
  int port_needs_uptime=0, port_needs_capacity=1;
  time_t now = time(NULL);
  int flags = 0;
//-------------------------------------------
  int ii = 0;
  for (circ=global_circuitlist;circ;circ = circ->next) {
      //-log_info(LD_CIRC,"\n\n Hey Mohsen: circ->lat = %f, circ->lon =  %f",circ->lat,circ->lon); 
      ii++;
  }
  log_info(LD_CIRC,"Hey Mohsen: we have %d circuits all in all",ii); 
...
//----------------------------------------------------
  ...
  //log_info(LD_CIRC,"Hey Mohsen: we have %d circuits and %d internal circuits",num, num_internal); 
  /* If that's enough, then stop now. */
  if (num >= 17)
    return; /* we already have many, making more probably will hurt */
  
  smartlist_t *dest = rep_hist_get_predicted_destination(); 
  SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {
      i = 0;
      j++;
//-log_info(LD_CIRC,"Hey Mohsen: added lat %f : added lon %f", pd->lat,pd->lon);
      while (circuit_stream_is_being_handled_Mohsen(pd->lat,pd->lon, MIN_NUMBER_OF_CIRC_FOR_EACH_DEST) < MIN_NUMBER_OF_CIRC_FOR_EACH_DEST){
           circuit_launch_Mohsen(CIRCUIT_PURPOSE_C_GENERAL, flags,pd->lat,pd->lon,MYLAT,MYLON);
           i++;
      }
     // log_info(LD_CIRC,"\n\n Hey Mohsen: i = %d",i);
  } SMARTLIST_FOREACH_END(pd);

  smartlist_free(dest);
//----------------------------------------------------
  ii = 0;
  //FILE * circuits;
  //circuits = fopen("./result/circuits.txt","w");
  for (circ=global_circuitlist;circ;circ = circ->next) {
      log_info(LD_CIRC,"Hey Mohsen: circ->lat = %f, circ->lon =  %f Exit = %s  Middle = %s  Guard = %s",circ->lat,circ->lon,circ->exit_ip,circ->middle_ip,circ->entry_ip); 
      //fprintf(circuits,"%f;%f;%s;%s;%s;%u;%ld\n",circ->lat,circ->lon,circ->exit_ip,circ->middle_ip,circ->entry_ip,circ->n_circ_id, circ->rtt);
      ii++;
  }
  //-log_info(LD_CIRC,"Hey Mohsen: we have %d circuits",ii); 
/*  if (ii < 12){
      circuit_launch_Mohsen(CIRCUIT_PURPOSE_C_GENERAL, flags,51,10,MYLAT,MYLON);
  }*/
//---------------------------------------------------
  //fclose(circuits); 
  /* Second, see if we need any more exit circuits. */
...
int
circuit_stream_is_being_handled_Mohsen(float lat, float lon, int min)
{
  circuit_t *circ;
  const node_t *exitnode;
  int num=0;
  ...
  for (circ=global_circuitlist;circ;circ = circ->next) {
    if (CIRCUIT_IS_ORIGIN(circ) &&
      ...
      if (origin_circ->unusable_for_new_conns)
        continue;
      if ((((circ->lat - lat) < 0.1) && ((circ->lat - lat) > -0.1)) && (((circ->lon - lon) > -0.1) && ((circ->lon - lon) < 0.1))){
	num++;
/*        if (num >= min)	
log_info(LD_CIRC,"\nHey Mohsen HERE\n");  
            return 1;*/
...
//Mohsen 25 Feb 2 PM--------------------------------------------
    circuit_predict_and_launch_new_Mohsen();
}
//---------------------------------------------------------------------
}
...
//Mohsen 25 Feb 2PM---------------------------------------------
origin_circuit_t *
circuit_launch_Mohsen(uint8_t purpose, int flags, float lat, float lon, float Mylat, float Mylon)
{
  origin_circuit_t *circ = circuit_launch_by_extend_info_Mohsen(purpose, NULL, flags, lat, lon, Mylat, Mylon);
  if ((lat -Mylat)>-0.001 && (lat -Mylat) < 0.001 && circ != NULL && (lon -Mylon)>-0.001 && (lon -Mylon) < 0.001){
     circ->around_me = 1;
     circ->base_.around_me = 1;
  } else {   
     circ->around_me = 0;
     circ->base_.around_me = 0; 
  }  
  return circ;
}

origin_circuit_t *
circuit_launch_by_extend_info_Mohsen(uint8_t purpose,
                              extend_info_t *extend_info,
                              int flags,float lat, float lon, float Mylat, float Mylon)
{
...
//    log_fn(LOG_INFO,"%d failures so far, not trying.",n_circuit_failures);
  ...
  /* try a circ. if it fails, circuit_mark_for_close will increment
   * n_circuit_failures */
  return circuit_establish_circuit_Mohsen(purpose, extend_info, flags,lat, lon, Mylat, Mylon);
}
...
//Mohsen----------------------------------------
  tor_addr_t addr_moh;
  if (tor_addr_parse(&addr_moh, conn->socks_request->address)>0){
     log_debug(LD_CIRC, "The address is an IP type = %s",conn->socks_request->address); 
  } else { 
     hostname_port *hp = find_hostname(NULL, conn->socks_request->address, 1);
     if (hp !=NULL){
         strlcpy(conn->socks_request->address ,hp->IP, sizeof(hp->IP));
         log_debug(LD_CIRC, "The address is a HOSTNAME type (equal IP %s ) and we found a match",conn->socks_request->address);
     } else {
           log_debug(LD_CIRC, "The address is a HOSTNAME type  %s and we could NOT find a match",conn->socks_request->address);
    }
}
//----------------------------------------------
...
//Mohsen-----------------------------------------------------
  ...
  if (tor_addr_parse(&addr_moh, conn->socks_request->address)>0){
     ...
     if (gir){
        ...
        smartlist_t *dest = rep_hist_get_predicted_destination();
        SMARTLIST_FOREACH_BEGIN(dest, predicted_destination *, pd) {
           tmp = compute_distance(pd->lat, pd->lon,gir->latitude, gir->longitude);
           if (min > tmp) {
                 min = tmp;
	         lat = pd->lat;
                 lon = pd->lon;
           }//if   
        }SMARTLIST_FOREACH_END(pd);//each	
        smartlist_free(dest);
    }else {
        lat = MYLAT;
        lon = MYLON;
...
//--------------------------------------------------------------
     //-log_info(LD_CIRC," Hey Mohsen:We are trying to build a circuit to lat %f and lon %f", lat, lon); 
      circ = circuit_launch_by_extend_info_Mohsen(new_circ_purpose, extend_info,
                                           flags, lat,lon,MYLAT,MYLON);
    }
...
connection_ap_handshake_attach_chosen_circuit(entry_connection_t *conn,
                                              ...
                                              crypt_path_t *cpath)
{
    log_debug(LD_APP|LD_CIRC,"circ->base_.state %u",circ->base_.state);
