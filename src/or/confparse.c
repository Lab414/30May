...
config_get_lines(const char *string, config_line_t **result, int extended)
{
  ...
  do {
    ...
    if (k && v) {
       ...
       * n^2 performance. */
//Mohsen --------------------------------------------------
      if (strcmp(k, "LAT") && strcmp(k, "LON") && strcmp(k, "MINGAIN") && strcmp(k, "MAX_BETA") && strcmp(k, "BETA") && strcmp(k, "P_PARAM") && strcmp(k, "ALPHA_MOHSEN") && strcmp(k, "LAMBDA") && strcmp(k, "MIN_NUMBER_OF_CIRC_FOR_EACH_DEST") )
 {
//Mohsen ------------------------
...
//Mohsen---------------------------------------------------------------
int
config_get_lines_Mohsen(const char *string, int extended)
{
  ...
  do {
    ...
    if (k && v) {
      ...
      if (!strcmp(k, "MINGAIN"))
         MINGAIN = atof(v);
      if (!strcmp(k, "MAX_BETA"))
         MAX_BETA = atof(v);
      if (!strcmp(k, "BETA"))
         BETA = atof(v);
      if (!strcmp(k, "P_PARAM"))
         P_PARAM = atof(v);
      if (!strcmp(k, "ALPHA_MOHSEN"))
         ALPHA_MOHSEN = atof(v);
      if (!strcmp(k, "LAMBDA"))
         LAMBDA = atof(v);
      if (!strcmp(k, "MIN_NUMBER_OF_CIRC_FOR_EACH_DEST"))
         MIN_NUMBER_OF_CIRC_FOR_EACH_DEST = (int)atof(v);
      tor_free(k);
      tor_free(v);
    } else {
      tor_free(k);
 ...
 */
// Mohsen---------------------------------------
void print_parameter_Mohsen(void){
 log_info(LD_CIRC,"Hey Mohsen MYLAT %f MYLON %f MINGAIN %f MAX_BETA %f BETA %f P_PARAM %f ALPHA_MOHSEN %f LAMBDA %f NIM_NUMBER_OF_ %d",MYLAT,MYLON,MINGAIN, MAX_BETA, BETA, P_PARAM, ALPHA_MOHSEN, LAMBDA, MIN_NUMBER_OF_CIRC_FOR_EACH_DEST);
return;
}
//----------------------------------------------------------------
void
