...
{
  tor_assert(conn);
//-log_debug(LD_APP,"Hey Mohsen: connection_edge_process_inbuf %s",safe_str_client(EDGE_TO_ENTRY_CONN(conn)->socks_request->address));
...
connection_ap_handshake_process_socks(entry_connection_t *conn)
{
  ...
  int had_reply = 0;
  connection_t *base_conn = ENTRY_TO_CONN(conn);
//-log_debug(LD_APP,"Hey Mohsen: connection_ap_handshake_process_socks %s",safe_str_client(conn->socks_request->address));
  tor_assert(conn);
  tor_assert(base_conn->type == CONN_TYPE_AP);
...
//Mohsen 21 March-------------------------------------------------------------
  tor_addr_t addr_moh;
  if (tor_addr_parse(&addr_moh, ap_conn->socks_request->address)>0){
     // the address is IP type
     //-log_info(LD_DIR,"Hey Mohsen: Add = %s ID = %d \n\n\n",ap_conn->socks_request->address, edge_conn->stream_id);
  } else {  // the address is Hostname type
     //-log_info(LD_DIR,"Hey Mohsen: Address = %s ID = %d \n",ap_conn->socks_request->address, edge_conn->stream_id);
     add_host_port_list(edge_conn->stream_id, ap_conn->socks_request->address, NULL, 0.0, 0.0, ap_conn->socks_request->port, 0);
  } 
//----------------------------------------------------------------------------
...
//Mohsen---------------------------------------------
  if (begin_type == RELAY_COMMAND_BEGIN){
  edge_conn->flag_begin = 1;
  tor_gettimeofday(&edge_conn->begin_time);//Mohsen 
  } 
//------------------------------------------------------------------
