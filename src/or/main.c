...
float MYLAT =-32; //Mohsen
float MYLON = 97;//Mohsen
float MINGAIN = 0.25;
float MAX_BETA = 20;
float BETA = 16;
float P_PARAM = 1.9;
float ALPHA_MOHSEN = .1;
float LAMBDA = .1;
...
parse_my_location_Mohsen(argc, argv);//Mohsen
...
//Mohsen---------------------------------
void parse_my_location_Mohsen(int argc, char **argv){


    char *cf=load_torrc_from_disk(argc, argv, 0);
    int retval;

    retval = config_get_lines_Mohsen(cf, 1);
    if (retval < 0) {
       log_warn(LD_CONFIG, "Hey Mohsen: ERROR!!! unable to read torrc");
    }
    tor_free(cf);
}
