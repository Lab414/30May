...
// Mohsen 4 Feb 1:07 PM-----------------------
node_get_address_string(node, address_buf, sizeof(address_buf));
gir = GeoIP_record_by_name(gi, address_buf);
//log_info(LD_FS, "Hey Mohsen! IP = %s Lat = %f & Lon = %f, count = %d  " ,address_buf, gir->latitude, gir->longitude, cnt++);
if (gir != NULL) {
node->lat =  gir->latitude;
node->lon =  gir->longitude;
}
else{
node->lat =  MYLAT;
node->lon =  MYLON;
