...
#include "rephist.h"//Mohsen added
...
//Mohsen-----------------------------------------
    if (conn->flag_begin == 1){
    struct timeval t2;
    tor_gettimeofday(&t2);
    conn->rtt = tv_mdiff(&conn->begin_time, &t2);
      //FILE * begin_connected_file;
      //begin_connected_file = fopen("./result/begin_connected.txt", "a");
      //fprintf(begin_connected_file, "%ld;%d;%u\n", conn->rtt, conn->stream_id,circ->n_circ_id);
        log_info(LD_CIRC,"Hey Mohsen: the time between begin_connected = %ldmsec, streamID %d CircID %u.", conn->rtt, conn->stream_id,circ->n_circ_id);
     //fclose(begin_connected_file);
     }//if
//----------------------------------------------------
    ...
    if (connected_cell_parse(rh, cell, &addr, &ttl) < 0) {
      ...
      connection_mark_unattached_ap(entry_conn, END_STREAM_REASON_TORPROTOCOL);
           ...
           //log_info(LD_DIR,"Hey Mohsen: up to hereeeeeeeeeeeeeee";
//Mohsen--------------------------------------------------------------------
      char address_buf[INET_NTOA_BUF_LEN+1];
      tor_addr_to_str(address_buf, &addr, sizeof(address_buf), 0);
      hostname_port *hp = find_hostname(conn->stream_id, entry_conn->socks_request->address, 0);
      if (hp != NULL){

	   strlcpy(hp->IP, address_buf, sizeof(hp->IP));  
           hp->flag = 1;
           log_info(LD_DIR,"Hey Mohsen: we mapped Host: %s to IP %s on stream %d", hp->host, hp->IP, hp->ID);
     }
...
//Mohsen ------------------------------------------------------------
        (circ->no_extended)++;
      //-log_debug(domain,"Hey Mohsen: extended flag  = %d",(circ->no_extended));
//-------------------------------------------------------------------
...
//Mohsen--------------------------------------------------
        struct timeval t2;
        tor_gettimeofday(&t2);
        long rtt = tv_mdiff(&circ->the_100th_cell, &t2);         
          //FILE * recv_sendme_file;
          //recv_sendme_file = fopen("./result/recv_sendme.txt", "a");
        (circ->no_sendme)++;
        log_info(LD_CIRC,"Hey Mohsen: the received sendmes = %d circID %u rtt %ld", circ->no_sendme, circ->n_circ_id, rtt); 
        //fprintf(recv_sendme_file, "%d;%ld;%u\n", circ->no_sendme,rtt, circ->n_circ_id) ;
        //fclose(recv_sendme_file);
                                  ...
                                  int *max_cells)
{
 ...
 repeat_connection_edge_package_raw_inbuf:
  ...
  if (!cpath_layer) { /* non-rendezvous exit */
    ...
    circ->number_of_sent_DATA++;
       //-log_debug(domain,"Hey Mohsen: the number of sent DATA %d and package_window %d CircID %u",circ->number_of_sent_DATA, cpath_layer->package_window, circ->n_circ_id); 
    if ( circ->number_of_sent_DATA == 100 ){
       tor_gettimeofday(&circ->the_100th_cell);
       circ->number_of_sent_DATA = 0;
    }
  }
...
//Mohsen--------------------------------------------------------
    if (circ->sendme_flag == 1){
        tor_gettimeofday(&circ->sendme1);
        circ->sendme_flag = 0;
        //-log_info(LD_CIRC,"Hey Mohsen: sendme1 set"); 
        (circ->counter)++;      
    } else {
      //FILE * sendme_file;
      //sendme_file = fopen("./result/sendme.txt", "a");
      circ->sendme_flag = 0;
      tor_gettimeofday(&circ->sendme2);
        circ->congestion = tv_mdiff(&circ->sendme1, &circ->sendme2); 
        (circ->counter)++;  
        circ->sendme1 = circ->sendme2;    
        if (circ->congestion > 0){
        log_info(LD_CIRC,"Hey Mohsen: the time between sendmes = %ldmsec, # of sent sendmes = %ld circID %u.", circ->congestion, circ->counter, circ->n_circ_id); 
        //fprintf(sendme_file, "%ld;%ld;%u\n", circ->counter,circ->congestion, circ->n_circ_id) ;
        //fclose(sendme_file);
        } else {
        log_info(LD_CIRC,"Hey Mohsen: ERROR time less than zero");
        }   
   } 
//--------------------------------------------------------------
