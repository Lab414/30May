...
#include "ht.h"
#include <GeoIP.h>//Mohsen
#include <GeoIPCity.h>//Mohsen

static void bw_arrays_init(void);
static void predicted_ports_init(void);
//Mohsen-----------------------------------------

static void host_port_list_init (void);
extern float MYLAT;
extern float MYLON;
//-----------------------------------------------
...
//Mohsen---------------------------------------
predicted_destination_init();//Mohsen
  host_port_list_init();
}
...
static smartlist_t *predicted_destination_list=NULL;

void
add_predicted_destination(float lat, float lon, int num)
{

  predicted_destination *pd = tor_malloc(sizeof(predicted_destination));
  pd->lat = lat;
  pd->lon = lon;
  pd->num = num;
  pd->sum_lat = 0;
  pd->sum_lon = 0;
  pd->total = 0;
  rephist_total_alloc += sizeof(*pd);
  smartlist_add(predicted_destination_list, pd);
}

void
predicted_destination_init(void)
{
  predicted_destination_list = smartlist_new();
  add_predicted_destination(34,-118,0);
  add_predicted_destination(40,-73,0);
  add_predicted_destination(51,10,0);
  add_predicted_destination(30,119,0);
  add_predicted_destination(MYLAT,MYLON,0);//Mohsen
}

static void
predicted_destination_free(void)
{
  rephist_total_alloc -=
    smartlist_len(predicted_destination_list)*sizeof(predicted_destination);
  SMARTLIST_FOREACH(predicted_destination_list, predicted_destination *,
                    pd, tor_free(pd));
  smartlist_free(predicted_destination_list);
}
//---------------------------------------------------------------------
//Mohsen: host_port list-----------------------------------------------
static smartlist_t *host_port_list=NULL;

void
add_host_port_list(streamid_t ID, char *hostname, char *IP, float lat, float lon, uint16_t port, int flag)
{ 
  hostname_port *hp = tor_malloc(sizeof(hostname_port));
  strlcpy(hp->host, hostname, sizeof(hp->host));  
  hp->port = port;
  if (IP !=NULL){ 
     strlcpy(hp->IP, IP, sizeof(hp->IP));
  }
  hp->flag = flag;
  hp->lat = lat;
  hp->lon = lon;
  hp->ID = ID;
  rephist_total_alloc += sizeof(*hp);
  smartlist_add(host_port_list, hp);
return;
}

static void host_port_list_init(void)
{
  host_port_list= smartlist_new();
  
}


static void
host_port_list_free(void)
{
  rephist_total_alloc -=
  smartlist_len(host_port_list)*sizeof(hostname_port);
  SMARTLIST_FOREACH(host_port_list, hostname_port *,
                    hp, tor_free(hp));
  smartlist_free(host_port_list);
}

hostname_port *find_hostname(streamid_t ID, char *host, int flag){
   
  //smartlist_t *list = get_host_port_list(); 
  SMARTLIST_FOREACH_BEGIN(host_port_list, hostname_port *, hp) {
   if (ID == NULL) {
     if ((strcmp(hp->host,host) == 0) && hp->flag == flag)
        return hp; 
   } else {
     if ((strcmp(hp->host,host) == 0) && hp->flag == flag && hp->ID == ID)
        return hp;
   }  
  } SMARTLIST_FOREACH_END(hp);
  return NULL;
}
//Mohsen: update the destinations  --------------------------
...
void update_destinations(void){
  ...
  double tmp; 
  predicted_destination *best_dest;
  gi = GeoIP_open("/home/mm/GeoIP-1.4.6/data/GeoLiteCity.dat", GEOIP_INDEX_CACHE);

  FILE *rececntly_seen_loc;
  FILE *center;
  rececntly_seen_loc = fopen("./result/rececntly_seen_loc.txt", "w");
  center = fopen("./result/destination_centers.txt", "w");

  SMARTLIST_FOREACH_BEGIN(list, hostname_port *, hp) {
     if (hp->flag == 1) {
        gir = GeoIP_record_by_name(gi, hp->IP);
        if (gir == NULL){
           continue;
        }else {
            SMARTLIST_FOREACH_BEGIN(predicted_destination_list, predicted_destination *, pd) {
     ...
     L++;
     best_dest->sum_lat += gir->latitude;
     best_dest->sum_lon += gir->longitude;
     (best_dest->total)++;
     fprintf(rececntly_seen_loc, "%f;%f\n", gir->latitude,gir->longitude); 
     GeoIPRecord_delete(gir);
     }//if

  }SMARTLIST_FOREACH_END(hp); //each

  smartlist_free(list);

  //GeoIPRecord_delete(gir);
  GeoIP_delete(gi);
  int l = smartlist_len(predicted_destination_list); 
  int i = 0;       
     fprintf(rececntly_seen_loc, "--------------------------------------------------\n");      
  SMARTLIST_FOREACH_BEGIN(predicted_destination_list, predicted_destination *, pd) {
     i++;
     pd->num = ((L)?(float)(pd->total)/((float)L) : 0) * 100;
     log_info(LD_CIRC,"new dest %d lat %f lon %f precent %f out of %d dests", i, pd->lat, pd->lon, pd->num, L);
     fprintf(center, "%f;%f;%d\n", pd->lat, pd->lon, pd->total); 
     if (i != l){  
        pd->lat = (pd->lat + pd->sum_lat)/((float)(pd->total+1));
        pd->lon = (pd->lon + pd->sum_lon)/((float)(pd->total+1));
     }//if
     ...
     log_info(LD_CIRC,"here? 3");
     fclose(rececntly_seen_loc);
     fclose(center);
  return;
}
...
//up to here----------------------
smartlist_t *
rep_hist_get_predicted_destination(void)
{
  smartlist_t *out = smartlist_new();

  /* clean out obsolete entries */
  SMARTLIST_FOREACH_BEGIN(predicted_destination_list, predicted_destination *, pd) {
      smartlist_add(out,pd );
  } SMARTLIST_FOREACH_END(pd);
  return out;
}

smartlist_t *
get_host_port_list (void)
{
  smartlist_t *out = smartlist_new();

  /* clean out obsolete entries */
  SMARTLIST_FOREACH_BEGIN(host_port_list, hostname_port *, pd) {
      smartlist_add(out,pd );
  } SMARTLIST_FOREACH_END(pd);
  return out;
}
