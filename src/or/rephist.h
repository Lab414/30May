...
typedef struct hostname_port{
  char host[MAX_SOCKS_ADDR_LEN];
  char IP[MAX_SOCKS_ADDR_LEN];
  uint16_t port;
  int flag;
  float lat;
  float lon;
...
smartlist_t *rep_hist_get_predicted_destination(void);//Mohsen
smartlist_t *get_host_port_list(void);//Mohsen
void update_destinations(void);//Mohsen
void predicted_destination_init(void);//Mohsen
void add_predicted_destination(float lat, float lon, int num);//Mohsen
void add_host_port_list(streamid_t ID, char *hostname, char *IP, float lat, float lon, uint16_t port, int flag);//Mohsen
hostname_port *find_hostname(streamid_t ID, char *host, int flag);//Mohsen
