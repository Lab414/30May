...
//Mohsen 4 Feb---------------------
static int compute_weighted_bandwidths_Mohsen(const smartlist_t *sl,
                            bandwidth_weight_rule_t rule,
                            u64_dbl_t **weights_out, float lat, float lon, float exit_lat, float exit_lon);
static const node_t *
smartlist_choose_node_by_bandwidth_weights_Mohsen(const smartlist_t *sl,
                                           bandwidth_weight_rule_t rule, float lat, float lon, float exit_lat, float exit_lon);
...
// Mohsen: 4 Feb------------------------------------------
const node_t *
node_sl_choose_by_bandwidth_Mohsen(const smartlist_t *sl,
                            bandwidth_weight_rule_t rule,float lat, float lon, float exit_lat, float exit_lon)
{ /*XXXX MOVE */
  const node_t *ret;
  ret = smartlist_choose_node_by_bandwidth_weights_Mohsen(sl, rule,lat,lon, exit_lat, exit_lon);
  return ret;
}
...
smartlist_choose_node_by_bandwidth_weights_Mohsen(const smartlist_t *sl,
                                           bandwidth_weight_rule_t rule, float lat, float lon, float exit_lat, float exit_lon)
{
  u64_dbl_t *weights=NULL;

  if (compute_weighted_bandwidths_Mohsen(sl, rule, &weights,lat,lon, exit_lat, exit_lon) < 0)
    return NULL;
...
//                              &sl_last_total_weighted_bw);
  ...
  {
    int idx = selected_index(weights, smartlist_len(sl));
    tor_free(weights);
...
double  compute_distance(float lat1, float lon1, float lat2, float lon2){
    double dLat,dLon,a, c,sinLat,sinLon;
    ...
    sinLat = sin(dLat/2);
    sinLon = sin(dLon/2); 
    a = sinLat * sinLat + sinLon * sinLon * cos(lat1) * cos(lat2); 
    c = 2 * atan2(sqrt(a), sqrt(1-a));
    return (6371 * c);
}
//--------------------------------------------------------------
int rand_sl(int n){
    float r = (float) (rand() % 100)/99;
    float  NewR=(1-pow(P_PARAM,(BETA*r)))*((1-MINGAIN)*BETA/(-MAX_BETA) + 1)/(1-pow(P_PARAM,BETA));
    return (int)(roundf(NewR*(n-1)));
}
//-------------------------------------------------------------------
...
{
    const circuit_idx *p1 = (circuit_idx *)v1;
    const circuit_idx *p2 = (circuit_idx *)v2;
...
int selected_index(const u64_dbl_t *entries, int n)
{
  int i = 0;
  weight_idx *w_i;
  //w_i = (weight_idx*) malloc (n*sizeof(weight_idx));
  w_i = tor_malloc_zero(sizeof(weight_idx)*n);
  if (w_i == NULL) exit (1);

  for (i=0; i<n; i++){
    w_i[i].idx=i;
    w_i[i].weight=entries[i].dbl;
};


qsort (w_i, n, sizeof(weight_idx), compare);
...
double compute_weight(double bw,double distance, double avg_bw, double avg_distance){
    
distance = distance / avg_distance;  
bw = bw / avg_bw;   
double D = 10/((distance)*(distance)+1);
double BW = 6.366*atan(bw);
double weight=(1-ALPHA_MOHSEN)*D + ALPHA_MOHSEN*BW;
    return weight;
}
//-------------------------------------------------------------
static int
compute_weighted_bandwidths_Mohsen(const smartlist_t *sl,
                            bandwidth_weight_rule_t rule,
                            u64_dbl_t **weights_out, float lat, float lon, float exit_lat, float exit_lon)
{
  u64_dbl_t *bandwidths;
  u64_dbl_t *distances;
  u64_dbl_t *weights;
  FILE *BWs_and_distances;
  //BWs_and_distances = fopen("./result/BWs_and_distances.csv","a");
  /* Can't choose exit and guard at same time */
  tor_assert(rule == NO_WEIGHTING ||
             rule == WEIGHT_FOR_EXIT ||
             rule == WEIGHT_FOR_GUARD ||
  ...
  //log_info(LD_CIRC,"lat1 %f lon1 %f lat2 %f lon2 %f",lat, lon, exit_lat, exit_lon);
  bandwidths = tor_malloc_zero(sizeof(u64_dbl_t)*smartlist_len(sl));
  distances = tor_malloc_zero(sizeof(u64_dbl_t)*smartlist_len(sl));
  weights = tor_malloc_zero(sizeof(u64_dbl_t)*smartlist_len(sl));
  // Cycle through smartlist and total the bandwidth.
  SMARTLIST_FOREACH_BEGIN(sl, const node_t *, node) {
    int this_bw = 0, is_me = 0;
    if (node->rs) {
      if (!node->rs->has_bandwidth) {
        tor_free(bandwidths);
        tor_free(distances);
    ...
    bandwidths[node_sl_idx].dbl = this_bw + 0.5;


if ((exit_lat > -0.000001 && exit_lat < 0.000001) && (exit_lon > -0.000001 && exit_lon < 0.000001)){
    distances[node_sl_idx].dbl = compute_distance(node->lat, node->lon, lat, lon);
 //log_debug(LD_CIRC, "Hey Mohsen: Here");
}
else {
distances[node_sl_idx].dbl = compute_distance(node->lat, node->lon, lat, lon) + compute_distance(node->lat, node->lon, exit_lat, exit_lon);
...
// Mohsen: 4 Feb ---------------
double avg_bw = 0, avg_distances = 0;
SMARTLIST_FOREACH_BEGIN(sl, const node_t *, node) {
avg_bw += bandwidths[node_sl_idx].dbl;
avg_distances += distances[node_sl_idx].dbl;

  } SMARTLIST_FOREACH_END(node);
int length = smartlist_len(sl);
avg_bw = avg_bw/length;
avg_distances = avg_distances /length; 
 log_debug(LD_CIRC, "average BW = %f and average distance = %f", avg_bw, avg_distances);
//-----------------------


SMARTLIST_FOREACH_BEGIN(sl, const node_t *, node) {
weights[node_sl_idx].dbl = compute_weight(bandwidths[node_sl_idx].dbl, distances[node_sl_idx].dbl, avg_bw, avg_distances);
    //fprintf(BWs_and_distances,"%f;%f;%f \n", bandwidths[node_sl_idx].dbl,distances[node_sl_idx].dbl,weights[node_sl_idx].dbl);    
  } SMARTLIST_FOREACH_END(node);
    //fprintf(BWs_and_distances,"----------------------------------------------------------------------------------\n");    
  *weights_out = weights;
   tor_free(bandwidths);
   tor_free(distances);
   //fclose(BWs_and_distances);
  return 0;
} 
///------------------------------------------------------------
...
// Mohsen 8 Feb------------------------------------------
const node_t *
router_choose_random_node_Mohsen(smartlist_t *excludedsmartlist,
                          routerset_t *excludedset,
                          router_crn_flags_t flags, float lat,float lon, float exit_lat, float exit_lon)
{ /* XXXX MOVE */
  const int need_uptime = (flags & CRN_NEED_UPTIME) != 0;
  const int need_capacity = (flags & CRN_NEED_CAPACITY) != 0;
  const int need_guard = (flags & CRN_NEED_GUARD) != 0;
...
/*  if ((r = routerlist_find_my_routerinfo()))
    routerlist_add_node_and_family(excludednodes, r);
*/
  router_add_running_nodes_to_smartlist(sl, allow_invalid,need_uptime, need_capacity,need_guard, need_desc);
  smartlist_subtract(sl,excludednodes);
  if (excludedsmartlist)
    smartlist_subtract(sl,excludedsmartlist);
  if (excludedset)
  ...
  // Always weight by bandwidth
  choice = node_sl_choose_by_bandwidth_Mohsen(sl, rule, lat,lon,0, 0);
  ...
  if (!choice && (need_uptime || need_capacity || need_guard)) {
    ...
    log_info(LD_CIRC,
             ...
             need_guard?", guard":"");
    flags &= ~ (CRN_NEED_UPTIME|CRN_NEED_CAPACITY|CRN_NEED_GUARD);
    choice = router_choose_random_node_Mohsen(
                     excludedsmartlist, excludedset, flags,lat,lon, 0, 0);
  }
  smartlist_free(excludednodes);
  if (!choice) {
    log_warn(LD_CIRC,
